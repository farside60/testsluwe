...
import io.reisub.devious.herblore.Potion;
import io.reisub.devious.herblore.Secondary;
...
public class HandleBank extends BankTask {
  ...
  @Inject
  private HandleBank(Herblore plugin) {
    this.plugin = plugin;
    setWaitTicks(3);
  }
  ...
  public boolean validate() {
    return plugin.isCurrentActivity(Activity.IDLE) && isLastBankDurationAgo(Duration.ofSeconds(2));
  }
  ...
  private void deposit() {
    switch (plugin.getConfig().task()) {
      case TAR_HERBS:
        SluweBank.depositAllExcept(false, ItemID.PESTLE_AND_MORTAR, ItemID.SWAMP_TAR);
        break;
      case PROCESS_SECONDARIES:
        SluweBank.depositAllExcept(false, ItemID.PESTLE_AND_MORTAR);
        break;
      case MAKE_COCONUT_MILK:
        SluweBank.depositAllExcept(false, ItemID.HAMMER, ItemID.IMCANDO_HAMMER);
        break;
      case MAKE_POTION:
        Item secondary = Inventory.getFirst(plugin.getConfig().potion().getSecondaryId());
        if (secondary != null && secondary.isStackable()) {
          SluweBank.depositAllExcept(false, plugin.getConfig().potion().getSecondaryId());
        } else {
