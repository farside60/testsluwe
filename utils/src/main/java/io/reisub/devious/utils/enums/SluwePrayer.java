...
public enum SluwePrayer {
  ...
  public static Set<SluwePrayer> getBestMeleeBuff() {
    ...
    if (level >= PIETY.getRequiredLevel() && pietyUnlocked) {
      ...
      Set<SluwePrayer> quickPrayers = new HashSet<>();

      SluwePrayer bestStrength = getBestStrength(level);
      SluwePrayer bestAttack = getBestAttack(level);
      SluwePrayer bestDefence = getBestDefence(level);
  ...
  private static SluwePrayer getBestAttack(int level) {
  ...
  private static SluwePrayer getBestStrength(int level) {
  ...
  private static SluwePrayer getBestDefence(int level) {
  ...
  public static Set<SluwePrayer> getBestRangedBuff() {
    final int level = Skills.getLevel(Skill.PRAYER);
    final boolean rigourUnlocked = Vars.getBit(Varbits.RIGOUR_UNLOCKED) != 0;
    final SluwePrayer bestDefense = getBestDefence(level);
  ...
  public static Set<SluwePrayer> getBestMagicBuff() {
    final int level = Skills.getLevel(Skill.PRAYER);
