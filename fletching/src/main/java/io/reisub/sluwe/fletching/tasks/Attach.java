...
public class Attach extends Task {
  ...
  private final Predicate<Item> featherlessPredicate =
      i ->
          i.getId() == ItemID.ARROW_SHAFT
              || i.getName().contains("dart tip")
              || i.getName().contains("bolts (unf)")
              || i.getId() == ItemID.UNFINISHED_BROAD_BOLTS;
  private final Predicate<Item> arrowheadsPredicate =
      i -> i.getId() == ItemID.BROAD_ARROWHEADS || i.getName().endsWith("arrowtips");
  ...
  public boolean validate() {
    return plugin.isCurrentActivity(Activity.IDLE)
        && ((Inventory.contains(ItemID.FEATHER) && Inventory.contains(featherlessPredicate))
            || (Inventory.contains(arrowheadsPredicate)
  ...
  public void execute() {
    ...
    if (config.product() == Product.BOLTS || config.product() == Product.DARTS) {
      for (int i = 0; i < 10; i++) {
      ...
      if (Time.sleepTicksUntil(() -> Players.getLocal().isAnimating(), 5)) {
        plugin.setActivity(Fletching.ATTACHING);
      }
    }
  }
  ...
  private void onStatChanged(StatChanged event) {
    if (event.getSkill() == Skill.FLETCHING && plugin.isCurrentActivity(Fletching.ATTACHING)) {
