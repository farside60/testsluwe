...
import io.reisub.devious.zulrah.rotationutils.ZulrahNpc;
import io.reisub.devious.zulrah.util.OverlayUtils;
...
public class SceneOverlay extends Overlay {
  ...
  private SceneOverlay(
      Client client, SluweZulrah plugin, ZulrahConfig config, SkillIconManager skillIconManager) {
    this.client = client;
    this.plugin = plugin;
    this.config = config;
                ...
                switch (config.phaseDisplayMode()) {
                  case CURRENT:
                    data.getCurrentZulrahNpc()
                        .ifPresent(
                            (npc) -> {
                              zulrahLocationsGrouped.put(
                                  npc.getZulrahLocation(), new MutablePair<>("Current", npc));
                            });
                    break;
                  case NEXT:
                    data.getNextZulrahNpc()
                        .ifPresent(
                            (npc) -> {
                              zulrahLocationsGrouped.put(
                                  npc.getZulrahLocation(),
  ...
  private void drawZulrahTile(
      Graphics2D graphics, ZulrahNpc zulrahNpc, String addonText, int offset) {
    if (zulrahNpc != null) {
      ...
      OverlayUtils.renderPolygon(
          graphics,
          tileAreaPoly,
          zulrahNpc.getType().getColor(),
          config.outlineWidth(),
          config.fillAlpha());
      ...
      if (basePoint != null) {
        ...
        if (this.config.phaseHats()) {
          BufferedImage icon =
              skillIconManager.getSkillImage(
                  zulrahNpc.getType().getSkill(), this.config.fontType() != FontType.BOLD);
    ...
    if (config.standLocations() || config.stallLocations()) {
              ...
              (data) -> {
                if (config.standLocations()) {
                  if (data.standLocationsMatch()) {
                    data.getCurrentDynamicStandLocation()
                        .ifPresent(
                            (loc) -> {
                              standLocationsGrouped.put(
                                  loc,
                    ...
                    data.getCurrentDynamicStandLocation()
                        .ifPresent(
                            (loc) -> {
                              standLocationsGrouped.put(
                                  loc, new MutablePair("Stand", config.standTileColor()));
                            });
                    data.getNextStandLocation()
                        .ifPresent(
                            (loc) -> {
                              standLocationsGrouped.put(
                                  loc, new MutablePair("Next", config.nextTileColor()));
                            });
                  }
                }

                if (config.stallLocations()) {
                  data.getCurrentStallLocation()
                      .ifPresent(
                          (loc) -> {
                            standLocationsGrouped.put(
                                loc, new MutablePair("Stall", config.stallTileColor()));
                          });
                  data.getNextStallLocation()
                      .ifPresent(
                          (loc) -> {
                            standLocationsGrouped.put(
                                loc, new MutablePair("Stall / next", config.nextTileColor()));
                          });
                }
              });
      ...
      while (location.hasNext()) {
        ...
        for (Iterator locationGrouped = standLocationsGrouped.get(standLocation).iterator();
            locationGrouped.hasNext();
            offset += graphics.getFontMetrics().getHeight()) {
          Pair pair = (Pair) locationGrouped.next();
          drawTile(
              graphics,
              standLocation.toLocalPoint(),
              (String) pair.getLeft(),
              (Color) pair.getRight(),
              offset);
        }
  ...
  private void drawTile(
      Graphics2D graphics, LocalPoint localPoint, String text, Color color, int offset) {
    if (localPoint != null && !Strings.isNullOrEmpty(text)) {
      ...
      OverlayUtils.renderPolygon(
          graphics, tilePoly, color, this.config.outlineWidth(), this.config.fillAlpha());
    }
  }
  ...
  private void renderPrayerConservation(Graphics2D graphics) {
    if (config.prayerConservation()) {
      ...
      if (player != null
          && (plugin.getZulrahNpc().getInteracting() == null
        ...
        Point textLocation =
            player.getCanvasTextLocation(
                graphics, "Turn off overheads to conserve prayer!", player.getLogicalHeight() - 22);
        Color color = Color.RED;
        ZulrahConfig config = this.config;
        Objects.requireNonNull(config);
        OverlayUtils.renderTextLocation(
            graphics,
            textLocation,
            "Turn off overheads to conserve prayer!",
            color,
            config::textOutline);
      }
    }
      ...
      sb =
          sb.append(
                  config.phaseTickCounter() && plugin.getPhaseTicks() >= 0
                      ...
                      : "")
              .append(
                  config.phaseTickCounter()
                          && config.attackTickCounter()
                          && plugin.getPhaseTicks() >= 0
                          && plugin.getAttackTicks() >= 0
                      ? " : "
                      : "")
              .append(
                  config.attackTickCounter() && plugin.getAttackTicks() >= 0
                      ? plugin.getAttackTicks()
                      : "");
        ...
        Point textLocation =
            plugin
                ...
                .getCanvasTextLocation(
                    graphics, sb.toString(), plugin.getZulrahNpc().getLogicalHeight() - 130);
        String string = sb.toString();
        Color tickCounterColors = config.tickCounterColors();
        ZulrahConfig config = this.config;
        Objects.requireNonNull(config);
        OverlayUtils.renderTextLocation(
            graphics, textLocation, string, tickCounterColors, config::textOutline);
      }
    }
  }
  ...
  private void renderZulrahTile(Graphics2D graphics) {
    if (config.displayZulrahTile()) {
      Polygon tileAreaPoly =
          Perspective.getCanvasTileAreaPoly(client, plugin.getZulrahNpc().getLocalLocation(), 5);
      OverlayUtils.renderPolygon(
          graphics,
          tileAreaPoly,
          config.zulrahTileColor(),
          config.outlineWidth(),
          config.fillAlpha());
    }
  }
    ...
    if (config.displayProjectiles() && plugin.getProjectilesMap().size() > 0) {
      plugin
          ...
          .forEach(
              (localPoint, ticks) -> {
                Point textLocation =
                    Perspective.getCanvasTextLocation(client, graphics, localPoint, "#", 0);
                String string = Integer.toString(ticks);
                Color color = ticks > 0 ? Color.WHITE : Color.RED;
                ZulrahConfig config = this.config;
                Objects.requireNonNull(config);
                OverlayUtils.renderTextLocation(
                    graphics, textLocation, string, color, config::textOutline);
                Polygon tilePoly = Perspective.getCanvasTilePoly(client, localPoint);
                OverlayUtils.renderPolygon(
                    graphics,
                    tilePoly,
                    this.config.projectilesColor(),
        ...
        .forEach(
            (obj, ticks) -> {
              ...
              OverlayUtils.renderPolygon(
                  graphics,
                  tileAreaPoly,
                  config.toxicCloudsColor(),
                  config.outlineWidth(),
                  config.fillAlpha());
              String ticksString = Integer.toString(ticks);
              Point textLocation =
                  Perspective.getCanvasTextLocation(client, graphics, localPoint, ticksString, 0);
              OverlayUtils.renderTextLocation(
                  graphics,
                  textLocation,
                  ticksString,
                  ticks > 0 ? Color.RED : Color.GREEN,
                  config::textOutline);
            });
  }
}
