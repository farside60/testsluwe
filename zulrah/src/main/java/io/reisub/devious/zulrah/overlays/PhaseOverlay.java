...
public class PhaseOverlay extends OverlayPanel {
  ...
  @Inject
  private PhaseOverlay(SluweZulrah plugin, ZulrahConfig config) {
    this.plugin = plugin;
    this.config = config;
  ...
  public Dimension render(Graphics2D graphics) {
    if (config.phaseDisplayType() == ZulrahConfig.DisplayType.OFF
        ...
        || plugin.getZulrahNpc() == null
        || plugin.getZulrahNpc().isDead()) {
      return null;
    }
              ...
              switch (config.phaseDisplayMode()) {
                ...
                default:
                  throw new IllegalStateException(
                      "[PhaseOverlay] Invalid 'phaseDisplayMode' config state");
              }
            });
    ...
    List<InfoBoxComponent> components =
        zulrahPhases.entries().stream().map(this::infoBoxComponent).collect(Collectors.toList());
  ...
  private void displayRotationName(Graphics2D graphics) {
    ...
    String text =
        plugin.getCurrentRotation() != null
            ? plugin.getCurrentRotation().getRotationName()
            : "Unidentified";
    TextComponent textComponent = new TextComponent();
    textComponent.setPosition(
        new Point(
            bounds.x + (bounds.width - graphics.getFontMetrics().stringWidth(text)) / 2,
            bounds.y + 1));
