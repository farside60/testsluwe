...
public class ZulrahData {
  @Nullable private final ZulrahPhase current;
  @Nullable private final ZulrahPhase next;
  ...
  public Optional<StandLocation> getCurrentDynamicStandLocation() {
    if (current == null) {
      ...
      switch (current.getAttributes().getStandLocation()) {
        case NORTHEAST_TOP:
          return SluweZulrah.isFlipStandLocation()
  ...
  public Optional<Prayer> getCurrentPhasePrayer() {
    if (SluweZulrah.isZulrahReset()) {
      ...
      Prayer invertedPhasePrayer =
          phasePrayer == Prayer.PROTECT_FROM_MAGIC
              ? Prayer.PROTECT_FROM_MISSILES
              : Prayer.PROTECT_FROM_MAGIC;
      return isJad()
          ? (SluweZulrah.isFlipPhasePrayer()
              ? Optional.of(invertedPhasePrayer)
              : Optional.of(phasePrayer))
          : Optional.of(phasePrayer);
    } else {
      return Optional.empty();
    }
  ...
  public boolean standLocationsMatch() {
    return getCurrentDynamicStandLocation().isPresent()
        && getNextStandLocation().isPresent()
        && (getCurrentDynamicStandLocation().get()).equals(getNextStandLocation().get());
  }

  public boolean stallLocationsMatch() {
    return isPhasesNotNull()
        && current.getAttributes().getStallLocation() != null
        && next.getAttributes().getStallLocation() != null
        && current
            .getAttributes()
            .getStallLocation()
            .equals(next.getAttributes().getStallLocation());
  }
